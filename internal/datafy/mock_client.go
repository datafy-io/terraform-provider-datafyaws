...
type MockClient struct {
	mu        sync.RWMutex
	ec2Conn   *ec2.EC2
	ec2Client *ec2_sdkv2.Client

	volumes         map[string]*Volume
	restoredVolumes map[string]*RestoredVolume
}

func NewMockClient(ec2Conn *ec2.EC2, ec2Client *ec2_sdkv2.Client) *MockClient {
	return &MockClient{ec2Conn: ec2Conn, ec2Client: ec2Client,
...
func (m *MockClient) CreateVolumeFromSnapshot(datafySnapshotId string, availabilityZone string, iops int32, throughput int32, tagz map[string]string) (*RestoredVolume, error) {
	...
	for i := 0; i < 2; i++ {
		cvo, err := m.ec2Client.CreateVolume(context.Background(), &ec2_sdkv2.CreateVolumeInput{
...
func (m *MockClient) AttachVolume(instanceId string, volumeId string, _ string) error {
	dvo, err := m.ec2Conn.DescribeVolumes(DescribeDatafiedVolumesInput(volumeId))
	if err != nil {
		return err
	...
	for _, id := range ids {
		err := func() error {
			...
			for _, dn := range []string{"/dev/sda", "/dev/sdb", "/dev/sdc", "/dev/sdd"} {
				if _, err := m.ec2Client.AttachVolume(context.Background(), &ec2_sdkv2.AttachVolumeInput{
					Device:     &dn,
	...
	return ec2_sdkv2.NewVolumeInUseWaiter(m.ec2Client).Wait(context.Background(), &ec2_sdkv2.DescribeVolumesInput{
		VolumeIds: ids,
	}, time.Minute)
}
...
func (m *MockClient) DetachVolume(instanceId string, volumeId string) error {
	...
	for _, id := range ids {
		if _, err := m.ec2Client.DetachVolume(context.Background(), &ec2_sdkv2.DetachVolumeInput{
			InstanceId: &instanceId,
			VolumeId:   &id,
			Force:      aws.Bool(true),
		}); err != nil {
	...
	return ec2_sdkv2.NewVolumeAvailableWaiter(m.ec2Client).Wait(context.Background(), &ec2_sdkv2.DescribeVolumesInput{
		VolumeIds: ids,
	}, time.Minute)
}
